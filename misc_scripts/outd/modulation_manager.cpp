...
class ExpandModulationButton : public OpenGlToggleButton {
  ...
  private:
    ...
    int num_sliders_;
    juce::Colour colors_[vital::kMaxModulationConnections];
...
ModulationAmountKnob::ModulationAmountKnob(juce::String name, int index) : SynthSlider(name),
...
void ModulationAmountKnob::setDestinationComponent(juce::Component* component, const std::string& name) {
  ...
  setPopupPrefix(vital::juce::Parameters::getDisplayName(name) + ": ");
...
vital::ModulationConnection* ModulationManager::getConnectionForModulationSlider(juce::Slider* slider) {
...
void ModulationManager::setModulationSliderScale(int index) {
  ...
  if (!connection->destination_name.empty()) {
    vital::ValueDetails details = vital::juce::Parameters::getDetails(connection->destination_name);
