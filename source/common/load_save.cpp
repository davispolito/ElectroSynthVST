...
#else
  //cuonfig_options.folderName = juce::String(ProjectInfo::projectName).toLowerCase();
#endif
...
//  juce::String xdg_data_home = SystemStats::getEnvironmentVariable ("XDG_DATA_HOME", {});
//
...
//    juce::File directory = home_directory.getChildFile("Music").getChildjuce::File("Vial");
//#else
...
void LoadSave::getAllUserPresets(juce::Array<juce::File>& presets) {
  ...
  getAllFilesOfTypeInDirectories(presets, juce::String("*.") + electrosynth::kPresetExtension, directories);
}
