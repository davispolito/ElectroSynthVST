...
//    }
    ...
    static Path clock() {
      ...
      Path path;
      path.addPieSegment(kBuffer, kBuffer, kClockWidth, kClockWidth, 0.0f, kClockAngle - 2.0f * electrosynth::kPi, 0.0f);
    ...
    static Path gear() {
      ...
      path.addPieSegment(offset, offset, diameter, diameter, 0.0f, 2.0f * electrosynth::kPi, 0.5f);
      for (int i = 0; i < kNumGearTeeth; ++i) {
        float phase = 2.0f * i * electrosynth::kPi / kNumGearTeeth;
    ...
    static Path magnifyingGlass() {
      ...
      path.addLineSegment(Line<float>(1.0f, 1.0f, 1.0f, 1.0f), 0.2f);
      float diameter = 2.0f * kRadius;
      path.addPieSegment(kGlassOffset, kGlassOffset, diameter, diameter, 0.0f, 2.0f * electrosynth::kPi, 1.0f - kWidthRatio);
    ...
    static Path halfSinCurve() {
      ...
      static constexpr float kFullRadians = electrosynth::kPi * 2.0f;
      ...
      for (int i = 0; i < kNumCurvePoints; ++i) {
        ...
        float y_t = sinf((t - 0.5f) * electrosynth::kPi) * 0.5f + 0.5f;
