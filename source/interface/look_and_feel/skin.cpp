...
void Skin::loadDefaultSkin() {
  MemoryInputStream skin((const void*)BinaryData::default_electrosynthskin, BinaryData::default_electrosynthskinSize, false);
...
//        FileChooser open_box("Open Skin", File(), String("*.") + electrosynth::kSkinExtension);
...
//        FileChooser save_box("Save Skin", File(), String("*.") + electrosynth::kSkinExtension);
//        if (save_box.browseForFileToSave(true))
//          skin_->saveToFile(save_box.getResult().withFileExtension(electrosynth::kSkinExtension));
